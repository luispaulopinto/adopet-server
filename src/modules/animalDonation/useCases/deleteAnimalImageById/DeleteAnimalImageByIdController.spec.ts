...
describe('Delete Animal Image By Id Controller', () => {
  const userData = {
    id: uuid(),
  ...
  it('should be able to delete animal image', async () => {
  ...
  it('should NOT be able to delete a non-existing animal image', async () => {
    const response = await request(app)
      .delete(
        `/donations/${animalData.id}/images/00000000-0000-0000-0000-000000000000`,
      )
      .set({
        Authorization: `Bearer ${token}`,
      });
  ...
  it('should NOT be able to delete an animal image not relate to the authenticated user', async () => {
  ...
  it('should NOT be able to delete an animal image without the authenticate token', async () => {
  ...
  it('should NOT be able to delete an animal image with an invalid authenticate token', async () => {
    const response = await request(app)
      .delete(`/donations/${animalData.id}/images/${animalImageData.id}`)
      .set({
        Authorization: `Bearer TOKEN`,
